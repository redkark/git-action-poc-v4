...
then
	...
	if ! git checkout --no-track -b $hfrnBranchName origin/release/next
	then
...
then
    ...
    while true; do
        read -p "$(tput setaf 6)Do you want generate PR for release/next branch of $BRANCH? yes/no: " prValid
        case $prValid in
            [Yy]* ) break;;
            [Nn]* ) exit;;
            * ) echo "Please answer yes or no.";;
